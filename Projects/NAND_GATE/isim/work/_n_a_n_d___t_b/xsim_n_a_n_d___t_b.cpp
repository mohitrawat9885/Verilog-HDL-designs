...
va_list vap)
{
    extern HSim__s6 * createworkM_n_a_n_d___t_b(const char*);
    HSim__s6 *blk = createworkM_n_a_n_d___t_b(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkM_n_a_n_d___g_a_t_e(const char*);
    HSim__s6 *blk = createworkM_n_a_n_d___g_a_t_e(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","NAND_TB", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","NAND_GATE", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_n_a_n_d___t_b(const char*);
        topvl = (HSim__s5*)createworkM_n_a_n_d___t_b("NAND_TB");
