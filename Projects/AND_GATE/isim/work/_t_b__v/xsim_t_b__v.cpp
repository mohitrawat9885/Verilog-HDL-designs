...
va_list vap)
{
    extern HSim__s6 * createworkM_t_b__v(const char*);
    HSim__s6 *blk = createworkM_t_b__v(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkM_a_n_d___g_a_t_e(const char*);
    HSim__s6 *blk = createworkM_a_n_d___g_a_t_e(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","TB_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","AND_GATE", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_t_b__v(const char*);
        topvl = (HSim__s5*)createworkM_t_b__v("TB_v");
