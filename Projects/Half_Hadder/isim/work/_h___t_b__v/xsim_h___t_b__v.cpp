...
va_list vap)
{
    extern HSim__s6 * createworkM_h___t_b__v(const char*);
    HSim__s6 *blk = createworkM_h___t_b__v(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkM_hald___adder(const char*);
    HSim__s6 *blk = createworkM_hald___adder(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","H_TB_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","Hald_Adder", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_h___t_b__v(const char*);
        topvl = (HSim__s5*)createworkM_h___t_b__v("H_TB_v");
