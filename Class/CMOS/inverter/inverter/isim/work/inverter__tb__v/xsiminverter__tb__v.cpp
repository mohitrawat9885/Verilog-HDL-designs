...
va_list vap)
{
    extern HSim__s6 * createworkMinverter__tb__v(const char*);
    HSim__s6 *blk = createworkMinverter__tb__v(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMinverter(const char*);
    HSim__s6 *blk = createworkMinverter(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","inverter_tb_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","inverter", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMinverter__tb__v(const char*);
        topvl = (HSim__s5*)createworkMinverter__tb__v("inverter_tb_v");
