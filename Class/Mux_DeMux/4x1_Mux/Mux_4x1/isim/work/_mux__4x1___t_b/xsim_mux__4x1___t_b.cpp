...
va_list vap)
{
    extern HSim__s6 * createworkM_mux__4x1___t_b(const char*);
    HSim__s6 *blk = createworkM_mux__4x1___t_b(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","Mux_4x1_TB", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","Mux_4x1", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_mux__4x1___t_b(const char*);
        topvl = (HSim__s5*)createworkM_mux__4x1___t_b("Mux_4x1_TB");
