...
va_list vap)
{
    extern HSim__s6 * createworkM_clock__v(const char*);
    HSim__s6 *blk = createworkM_clock__v(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMdff(const char*);
    HSim__s6 *blk = createworkMdff(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMsynchronous__counter(const char*);
    HSim__s6 *blk = createworkMsynchronous__counter(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","Clock_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","dff", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","synchronous_counter", (HSimInstFactoryPtr)IF2);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF3);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_clock__v(const char*);
        topvl = (HSim__s5*)createworkM_clock__v("Clock_v");
