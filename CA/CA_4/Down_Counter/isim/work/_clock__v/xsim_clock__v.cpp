...
va_list vap)
{
    extern HSim__s6 * createworkMt__ff(const char*);
    HSim__s6 *blk = createworkMt__ff(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkM_down___counter(const char*);
    HSim__s6 *blk = createworkM_down___counter(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","Down_Counter", (HSimInstFactoryPtr)IF2);
