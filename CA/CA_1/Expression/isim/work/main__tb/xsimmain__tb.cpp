...
va_list vap)
{
    extern HSim__s6 * createworkMmain__tb(const char*);
    HSim__s6 *blk = createworkMmain__tb(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMmain(const char*);
    HSim__s6 *blk = createworkMmain(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","main_tb", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","main", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMmain__tb(const char*);
        topvl = (HSim__s5*)createworkMmain__tb("main_tb");
