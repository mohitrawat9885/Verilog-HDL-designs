...
va_list vap)
{
    extern HSim__s6 * createworkMmytb(const char*);
    HSim__s6 *blk = createworkMmytb(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","mohit", (HSimInstFactoryPtr)IF2);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF3);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMmytb(const char*);
        topvl = (HSim__s5*)createworkMmytb("mytb");
