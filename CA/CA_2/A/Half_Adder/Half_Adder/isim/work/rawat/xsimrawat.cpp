...
va_list vap)
{
    extern HSim__s6 * createworkMrawat(const char*);
    HSim__s6 *blk = createworkMrawat(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","rawat", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","mohit", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMrawat(const char*);
        topvl = (HSim__s5*)createworkMrawat("rawat");
