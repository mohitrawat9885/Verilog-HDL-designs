...
va_list vap)
{
    extern HSim__s6 * createworkM_mux__16x1___t_b__v(const char*);
    HSim__s6 *blk = createworkM_mux__16x1___t_b__v(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkM_mux__16x1(const char*);
    HSim__s6 *blk = createworkM_mux__16x1(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","Mux_16x1_TB_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","Mux_4x1", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","Mux_16x1", (HSimInstFactoryPtr)IF2);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF3);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_mux__16x1___t_b__v(const char*);
        topvl = (HSim__s5*)createworkM_mux__16x1___t_b__v("Mux_16x1_TB_v");
